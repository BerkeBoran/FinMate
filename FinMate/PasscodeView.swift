...
//  FinMate
//
//  Şifre ekranı — üç modda kullanılır:
//  - .create  : Onboarding'de yeni şifre belirleme (iki kez gir + onayla)
//  - .change  : Ayarlardan şifre değiştirme (önce biyometri, sonra yeni şifre)
//  - .verify  : Açılışta kilidi açma (biyometri ile "Şifremi Unuttum" desteği)
//
...
struct PasscodeView: View {
    ...
    /// Şifre veya biyometri ile başarı. Create/change modunda yeni hash kaydedilmiş olarak çağrılır.
    var onSuccess: () -> Void
    /// İptal — verify modunda nil ise iptal butonu çıkmaz (kilit ekranı için).
    ...
    /// Cihazda Face ID / Touch ID destekli mi?
    ...
    var body: some View {
        ...
        .onAppear {
            // Change modunda biyometri varsa otomatik başlat
    ...
    // MARK: - Passcode Entry Screen (4-digit dots + number pad)
    ...
    // MARK: - Biometric Change Screen (.change modu)
    ...
    private func handleDigit(_ d: Int) {
        ...
        entered += "\(d)"
        if entered.count == 4 {
            // 4 hane tamamlandı, faza göre işle
    ...
    private func processCompleted() {
        ...
        case .biometricChange:
            // Bu faz şifre kabul etmez — sadece biyometri ile geçilir
            break
        ...
        case .confirmNew:
            ...
            // Yeni şifre aktif veya son 3 kullanılandan biri mi?
